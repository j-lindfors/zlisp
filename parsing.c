...
lval* builtin_load(lenv* e, lval* a) {
  ...
  mpc_result_t r;
  if (mpc_parse_contents(a->cell[0]->str, Zlisp, &r)) {
...
int main(int argc, char** argv) {
  ...
  /* Load standard lib */
  lval* stdlib = lval_add(lval_sexpr(), lval_str("stdlib.zl"));
  lval* z = builtin_load(e, stdlib);

  /* If the result is an error be sure to print it */
  if (z->type == LVAL_ERR) { lval_println(z); }
  
  
  /* Interactive Prompt */
  if (argc == 1) {
  
    puts("Zlisp Version 0.0.0.1.0");
